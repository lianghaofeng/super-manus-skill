...
The full row appended to `drift_log.md ## PRD drift` for branches (b)/(c)/(a→ii)/(a→iv) is:

```
| <YYYY-MM-DD> | <author> | <module> | (soft) spec-PRD topic-overlap: spec edit '<one-line>' shares vocabulary with PRD ## <section> | acknowledged-soft: <variant> |
```
