...
If `$override` is non-empty (`opus` / `sonnet` / `haiku`), pass `model: "$override"` to the Agent tool. If empty, omit and the agent's frontmatter applies: thinkers pinned to `opus`, writers `inherit` from main session. `effort:` is governed by `CLAUDE_CODE_EFFORT_LEVEL` env var (highest priority) → frontmatter (`max` for thinkers / `high` for writers) → model default; not configurable through `.super-manus/agents.yml`. Override config file lives at `.super-manus/agents.yml` (created by `/super-manus:start`). Same rules as `/super-manus:impl` — see that command for full details.
