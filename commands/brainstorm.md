---
description: PRD-led 6-question brainstorm — write prd/_index.md and per-module prd/<module>.md stubs, then seed the first MVP update folder
---
...
1. **`<feature>/prd/_index.md`** — replace the seeded placeholder with the user's answers.
...
2. **`<feature>/prd/<module>.md`** for each module — copy from `templates/prd_module.md`, substitute `<module name>` with the actual module name, and pre-fill:
...
>
> Next steps:
> 1. Edit `prd/<module>.md ## What users get` and `## How it connects` to flesh out the target state (you can do this any time).
