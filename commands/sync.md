...
Spawn the **`sync-planner`** agent (Agent tool, `subagent_type="super-manus:sync-planner"`). The persona ("senior tech lead"), source-priority hierarchy, and phase-decomposition rules live in [agents/sync-planner.md](../agents/sync-planner.md) — do NOT duplicate them here. The orchestrator only owns the inputs.
