#!/usr/bin/env bash
# Tests templates/drift_log.md — v0.9.5 R10 rename from prd_drift.md.
# Two H2 sections (## PRD drift / ## Spec drift), each carrying the same
# 4-column schema. The append-only / Resolution-only-mutable invariant is
# preserved verbatim from prd_drift.md.
...
# 4-column schema preserved (Date / Module / Conflict / Resolution).
# Schema appears under each H2 section — at least 2 occurrences expected.
header_count=$(grep -cF "| Date | Module | Conflict | Resolution |" "$F" || true)
[ "$header_count" -ge 2 ] \
  || { echo "FAIL: 4-column schema header must appear under each of the two H2 sections (found $header_count, need >=2)"; exit 1; }
