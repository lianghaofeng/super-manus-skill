...
grep -qF "auto-appended by post-commit hook" "$F" || { echo "FAIL: missing post-commit trigger comment"; exit 1; }
grep -qF "auto-appended by Stop hook" "$F" || { echo "FAIL: missing Stop hook trigger comment"; exit 1; }
