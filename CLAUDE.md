...
- **Current design**: `docs/design-v0.8.md` — read this before adding anything. Covers the v0.8.0 `/super-manus:reverse-prd` runtime probe (`scripts/probe-runtime.sh`) + Cross-validation protocol (3 new `(audit)` subtypes: `runtime-unverified` / `runtime-only` / `source-runtime-conflict`) + smart tool-budget formula `10 + 5×N + 10` (cap 60), plus per-agent model/effort routing (all 6 agents pinned to `model: opus`; thinkers `effort: max`, writers `effort: high`). The Docker startup gate (offering `docker compose up -d` to the user when compose services are stopped) goes through `AskUserQuestion`; the probe script itself is strictly read-only.
