...
- `## Reflections` (v0.7.4, simplified v0.9.8 R17): cross-phase memory within the current update, written ONLY by the `/super-manus:impl` orchestrator at phase close (skipped when a phase has zero reviewer RETURN events). Each entry is `### p<n>: <name>` followed by exactly three bullets — `Misstep:` (surface event), `Root cause:` (causal), `Heuristic:` (prescriptive rule for next phase). The next phase's `impl-architect` / `impl-test-writer` spawns include the section verbatim as `<update_reflections>` (loaded via `sm_load_update_reflections` — same-update only); writers honor Heuristic lines as checklist items. Cross-update memory (lessons applicable to all future modules / milestones) now flows through the `wiki/` layer instead — reviewer at pre-close optionally flags `wiki-candidates:` for generalizable lessons; user gate at orchestrator's promote step. Voice rule: Heuristics must be prescriptive ("Run head -1 on every declared input source before drafting") — if the line reads as a recap, it has drifted into `## Errors` / `## Session log` territory. Do NOT hand-edit.
...
End-of-update drift gate has **four passes** (v0.9.8 R19 — was 3 in v0.5–v0.9.7). Pass 1-3 are BLOCKING; Pass 4 is NON-BLOCKING (advisory):

- **Pass 1** — drift refresh + missing-spec detection. PRD-side conflicts → row in `## PRD drift`. `<module>.spec.md` missing (R7 required-mode) → row in `## Spec drift`.
- **Pass 2** — e2e coverage check: every touched `## What users get` capability needs `e2e/<module>/test_<capability>.<ext>` to exist and pass. Missing or red → `pending` row in `## PRD drift` (v0.9.5 R10 — was `prd_drift.md`), BLOCKS roadmap from flipping to `stable`.
- **Pass 3** — pending count summed across BOTH H2 sections of `drift_log.md` (`## PRD drift` + `## Spec drift`) must equal zero. Total > 0 → BLOCKED.
- **Pass 4** (v0.9.8 R19) — `/super-manus:wiki-lint` wiki health pass. Spawns `impl-reviewer` with `mode=wiki-lint`; reviewer runs five checks (contradiction / stale / orphan / gap / cross-ref miss) across `wiki/` and every `findings.md`, writes one entry to `wiki/_log.md`. **Does NOT fail-close the gate** — wiki rot is surfaced to the user but doesn't block the milestone close. Skipped entirely if `docs/super-manus/wiki/` is absent (pre-v0.9.8 project).
